...
azkaban.FlowTableView = Backbone.View.extend({
    events: {
        ...
        "click .runWithDownStream": "runWithDownStream",
    ...
    createJobListTable: function (data, innerTable) {
        ...
        var requestURL = contextURL + "/manager?project=" + project + "&jobEdit&job=";
        for (var i = 0; i < nodes.length; i++) {
            ...
            if ("leaf_job" == name) {
                console.log("叶子节点，在工作流中的作业列表中不显示");
                continue;
            }
            ...
            if (execAccess) {
                ...
                var divRunWithDownStream = document.createElement("button");
                $(divRunWithDownStream).attr('type', 'button');
                $(divRunWithDownStream).addClass("btn");
                $(divRunWithDownStream).addClass("btn-success");
                $(divRunWithDownStream).addClass("btn-xs");
                $(divRunWithDownStream).addClass("runWithDownStream");
                $(divRunWithDownStream).text("Run With DownStream");
                divRunWithDownStream.jobName = name;
                divRunWithDownStream.flowId = flowId;
                $(hoverMenuDiv).append(divRunWithDownStream);

                var divEditJob = document.createElement("a");
                $(divEditJob).attr("href", "/manager?project=" + project + "&job=" + name + "&jobEdit");
                $(divEditJob).addClass("btn");
                $(divEditJob).addClass("btn-primary");
                $(divEditJob).addClass("btn-xs");
                $(divEditJob).text("Edit Job");
                divEditJob.jobName = name;
                divEditJob.flowId = flowId;
                $(hoverMenuDiv).append(divEditJob);

                var divDeleteJob = document.createElement("a");
                $(divDeleteJob).attr("href", "/manager?project=" + project + "&job=" + name + "&jobDelete");
                $(divDeleteJob).addClass("btn");
                $(divDeleteJob).addClass("btn-danger");
                $(divDeleteJob).addClass("btn-xs");
                $(divDeleteJob).addClass("deleteJob");
                $(divDeleteJob).text("Delete Job");
                divDeleteJob.jobName = name;
                divDeleteJob.flowId = flowId;
                $(hoverMenuDiv).append(divDeleteJob);
    ...
    runWithDownStream: function (evt) {
        var jobId = evt.currentTarget.jobName;
        var flowId = evt.currentTarget.flowId;
        console.log("Run With DownStream");
