...
public class ProjectManagerServlet extends LoginAbstractAzkabanServlet {
    ...
    private static final String JOB_PATH_PREFIX = "sh /opt/dw/job/";
    private static final String JOB_PATH_SUFFIX = ".sh";
    private static final String JOB_DEFAULT_PARAMETER = "${date} ${hour}";

    private static final HashMap<Integer, String> warnLevelMap = new HashMap() {
        {
            put(1, "高");
            put(2, "中");
            put(3, "低");
    ...
    private static final HashMap<String, String> jobTypeMap = new HashMap() {
        {
            put("command", "Shell 脚本");
            put("hiveSql", "Hive Sql");
        }
    };
    ...
    protected void handleGet(final HttpServletRequest req, final HttpServletResponse resp,
        ...
        if (hasParam(req, "project")) {
            ...
            // ++++++
            // 创建作业
            if (hasParam(req, "jobCreate")) {
                handleJobCreatePage(req, resp, session);
            }
            // 修改作业
            else if (hasParam(req, "jobEdit")) {
                handleJobEditPage(req, resp, session);
            }
            // 删除作业
            else if (hasParam(req, "jobDelete")) {
                handleJobDeletePage(req, resp, session);
            }
            // 刷新项目的工作流，从数据库读取表信息，生成一个zip包，交给azkaban使用原生解析方法解析
            else if (hasParam(req, "refreshProject")) {
                handleRefreshProject(req, resp, session);
            }
            // ++++++
    ...
    private void handleAJAXAction(final HttpServletRequest req,
        ...
        if (project == null) {
            ...
            // ++++++
            // 保存作业，创建|编辑
            if (ajaxName.equals("saveJob")) {
                if (handleAjaxPermission(project, user, Type.WRITE, ret)) {
                    ajaxSaveJob(project, ret, req, user);
                }
            }
            // 删除作业
            //else if (ajaxName.equals("deleteJob")) {
            //    if (handleAjaxPermission(project, user, Type.WRITE, ret)) {
            //        ajaxDeleteJob(project, ret, req, user);
            //    }
            //}
            // ++++++

            else if (ajaxName.equals("getProjectId")) {
    ...
    private void handleJobPage(final HttpServletRequest req, final HttpServletResponse resp,
        ...
        try {
            ...
            final Props jobProp = this.projectManager.getProperties(project, flow, jobName, node.getJobSource());
            Props overrideProp = this.projectManager.getJobOverrideProperty(project, flow, jobName, node.getJobSource());
            if (overrideProp == null) {
    ...
    private Permission getPermissionObject(final Project project, final User user,
        ...
        for (final String roleName : user.getRoles()) {
            ...
            if (role == null)
                logger.warn("not have role " + roleName);
            else
    ...
    // +++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    // 进入创建作业页面
    private void handleJobCreatePage(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException {
        Page page = newPage(req, resp, session, "azkaban/webapp/servlet/velocity/jobcreatepage.vm");
        ...
        page.add("user", user);
        page.add("showName", "创建作业");
        page.add("saveType", "create");

        page.add("jobTypeMap", jobTypeMap);
        page.add("warnLevelMap", warnLevelMap);
    ...
    // 进入编辑作业页面
    private void handleJobEditPage(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException {
        Page page = newPage(req, resp, session, "azkaban/webapp/servlet/velocity/jobcreatepage.vm");
        String projectName = getParam(req, "project");
        ...
        page.add("user", user);
        page.add("showName", "编辑作业");
        page.add("saveType", "edit");
        ...
        // 通过JDBC获得作业信息
        Job job = projectManager.selectJob(projectName, jobName);
    ...
    // 进入删除作业页面
    private void handleJobDeletePage(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException, IOException {
        ...
        if (StringUtils.isBlank(projectName)) {
            ...
            int number = projectManager.deleteJob(projectName, jobName);
            if (number != 1)
                ret.put("error", "Delete job error,database return " + number + " row data");
        }

        if (ret.containsKey("error"))
            setErrorMessageInCookie(resp, ret.get("error"));
        else
            setSuccessMessageInCookie(resp, "Job " + projectName + "." + jobName + " delete success.");
    ...
    // 刷新项目
    private void handleRefreshProject(HttpServletRequest req, HttpServletResponse resp, Session session) throws ServletException, IOException {
        HashMap<String, String> ret = new HashMap<String, String>();
        String projectName = getParam(req, "project");
        ...
        if (StringUtils.isBlank(projectName)) {
            ...
            try {
                logger.info("刷新项目 " + projectName);
                File archiveFile = new File(tempDir, projectName + ".zip");

                final Props props = new Props();
                props.put(ValidatorConfigs.CUSTOM_AUTO_FIX_FLAG_PARAM, "true");

                projectManager.getJobsZipFile(project.getName(), archiveFile);
                final Map<String, ValidationReport> reports = projectManager.uploadProject(project, archiveFile, type, user, props);
        ...
        if (ret.containsKey("error"))
            ...
            setSuccessMessageInCookie(resp, "Project " + projectName + " refresh success");

        resp.sendRedirect(req.getContextPath() + "manager?project=" + projectName);
    }

    // 异步保存作业
    private void ajaxSaveJob(Project project, HashMap<String, Object> ret, HttpServletRequest req, User user)
            throws ServletException {
        Map<String, String> jobParamGroup = getParamGroup(req, "jobOverride");

        String jobName = jobParamGroup.get("job_name");
        String jobType = jobParamGroup.get("job_type");
        if (StringUtils.isBlank(jobName) || StringUtils.isBlank(jobType)) {
            ret.put("error", "作业名和作业类型不能为空");
            return;
        }

        String[] jobNameArray = jobName.split("_", 3);
        if (jobNameArray.length != 3) {
            ret.put("error", "作业名格式异常，未达到三段长度");
            return;
        }

        // shell 类型的作业，自动根据规范生成
        if ("command".equals(jobType)) {
            // ods_abc_dsfsdf
            // /opt/dw/job/ods/job/ods_abc_dsfsdf.sh
            String jobCommand = JOB_PATH_PREFIX + jobNameArray[0] + "/" + jobNameArray[1] + "/job/"
                    + jobName + JOB_PATH_SUFFIX
                    + " " + JOB_DEFAULT_PARAMETER;
            jobParamGroup.put("job_command", jobCommand);
        }

        jobParamGroup.put("user_id", user.getUserId());

        projectManager.saveJob(jobParamGroup);
    }
}
